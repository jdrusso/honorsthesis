...
def gillespie(mu1, mu2, alpha, t_max, q=False, s0 = S_0, r0 = R_0, _PLASMIDS=PLASMIDS, num=0):
    ...
    while t < t_max:
        ...
        if TRANSFORMATION and CONJUGATION:
            print("Pick either transformation or conjugation.")
            return -1
        ...
        if TRANSFORMATION:
            ...
            temp = N_s * (plasmids/_PLASMIDS) * alpha
        ...
        # Reaction 3: Y -> Y + X
        # HACK: Only need this condition to be met if we're doing conjugation.
        pop = True
        if CONJUGATION:
            pop = N_s >= 1
        elif r1 * a0 < sum(a[:3]) and pop:
            ...
            if TRANSFORMATION:
                ...
                if SYMMETRIC:
                    ...
                    # Asymmetric division - conserves plasmid number
                    N_s += 1.
...
def main():
    ...
    run(alphas=[a], mu1=.8, mu2s=[.00005]*10, t_max=35, filename=filename)
    return
